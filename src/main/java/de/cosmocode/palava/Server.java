...
public class Server extends Thread
{
	...
	public Server(Properties config, Properties alias, boolean interactive)
	{
		...
		{
			logger.fatal("Cannot resolve version: " + e);
			return;
		}
		// print license
	...
	public void run()
	{
		if ( interactive){
			...
			try {
				...
				logger.error("cannot consume request", e);
		...
		try {
			...
			logger.info("Server main-thread is listening on port " + listenPort);
			...
			logger.fatal("Server main-thread ended!", ioe);
			ioe.printStackTrace();
			shutdown();
		} finally {
			// give the Threads a chance to end
			try {
				logger.debug("No more connections; 5 seconds until program exits.");
				Thread.sleep(3000);
				clients.interrupt();
				Thread.sleep(2000);
			}
